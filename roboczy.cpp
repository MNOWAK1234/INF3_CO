...
#define POPULATION 6000   // Number of solutions per generation
#define LEAVE 5
#define STABLE 800
int  n;
...
{
    int vertices=(1+sqrt(8*(int)edges.size()+1))/2;//solved quadratic equation, calculates the number of vertexes and build square matrix
    vector<double>help;
    vector<vector<double> >distances;
    for(int i=0; i<vertices; i++)help.push_back(0);
    for(int i=0; i<vertices; i++)distances.push_back(help);//needs improvement, I don't get it how to make 2-dimensional vector using resize XD
...
void cycleCross()
{
    for(int i=TOP_SOLUTIONS; i<2*TOP_SOLUTIONS; i+=2)
    {
        ...
        points1=solutions[rand()%LEAVE+TOP_SOLUTIONS].getPath();
        points2=solutions[rand()%TOP_SOLUTIONS+TOP_SOLUTIONS].getPath();
...
void orderCross()
{
    for(int i=TOP_SOLUTIONS+1; i<2*TOP_SOLUTIONS; i+=2)
    {
        points1.clear();
        points2.clear();
        for(int j=0; j<=n; j++)
        ...
        int first=rand()%LEAVE+TOP_SOLUTIONS;
        int second=rand()%TOP_SOLUTIONS+TOP_SOLUTIONS;
        points1=solutions[first].getPath();
        points2=solutions[second].getPath();
        for(int j=0; j<n; j++) //position table
        {
            parent1[points1[j]]=j;
            parent2[points2[j]]=j;
...
int main()
{
    ...
    for(int k=0; k<4000; k++)
    {
        sort(solutions.begin(),solutions.end());
        tmp.clear();
        tmp.assign(solutions.begin(),solutions.begin()+6000);
        solutions=tmp;
